!function (t) {
    t.fn.persianDatepicker = function (a) {
        var s = this.data("persianDatepicker");
        return s ? !0 === a ? s : this : this.each(function () {
            return t(this).data("persianDatepicker", new e(this, a))
        })
    };
    var e = function () {
        function e(e, a) {
            ...
            if (s._persianDate = null != s.options.selectedDate ? (new persianDate).parse(s.options.selectedDate) : s.now(), null != i.selectableYears && -1 == i.selectableYears._indexOf(s._persianDate.year) && (s._persianDate.year = i.selectableYears[0]), -1 == s.options.selectableMonths._indexOf(s._persianDate.month) && (s._persianDate.month = i.selectableMonths[0]), s.persianDate = s._persianDate, s._id = "pdp-" + Math.round(1e7 * Math.random()), s.persianDate.formatDate = i.formatDate, s.calendar = t('<div id="' + s._id + '" class="pdp-' + i.theme + '" />'), null != s.options.startDate) {
                s.options.selectableYears = [];
                for (var r = s.persianDate.parse(s.options.startDate).year; r <= s.persianDate.parse(s.options.endDate).year; r++) s.options.selectableYears.push(r)
            }
            (n.attr("pdp-id") || "").length || n.attr("pdp-id", s._id), n.addClass("pdp-el").on("click", function (t) {
                s.show(t)
            }).on("focus", function (t) {
                s.show(t)
            }), i.closeOnBlur && n.on("blur", function (t) {
                s.calendar.is(":hover") || s.hide(t)
            }), i.selectedBefore && (null != s.options.selectedDate ? s.showDate(n, s.persianDate.parse(s.options.selectedDate).toString("YYYY/MM/DD/" + s.jDateFunctions.getWeekday(s.persianDate.parse(s.options.selectedDate)), s.now().gDate, i.showGregorianDate)) : s.showDate(n, s.now().toString("YYYY/MM/DD/" + s.jDateFunctions.getWeekday(s.now())), s.now().gDate, i.showGregorianDate)), i.isRTL && n.addClass("rtl"), s.calendar.length && !i.alwaysShow && s.calendar.hide(), t(document).bind("mouseup", function (e) {
                var a = e.target, o = s.calendar;
            ...
            var d = function () {
                var t = n.offset();
                s.calendar.css({
                    top: t.top + n.outerHeight() + i.calendarPosition.y + "px",
                    left: t.left + i.calendarPosition.x + "px"
                })
            };
        ...
        return e.prototype = {
            show: function () {
                this.calendar.show(), t.each(t(".pdp-el").not(this.el), function (t, e) {
                    e.length && e.options.onHide(e.calendar)
                }), this.options.onShow(this.calendar), this.onresize()
            }, hide: function () {
                this.options.onHide(this.calendar), this.options && !this.options.alwaysShow && this.calendar.hide()
            }, render: function () {
                this.calendar.children().remove(), this.header(), this.dows(), this.content(), this.footer(), this.options.onRender()
            }, header: function () {
                var e = this;
                ...
                var s = t('<ul class="monthSelect" ' + e.selectUlStyle + " />").hide(),
                    ...
                    o = t("<span/>").html(e.options.months[e.persianDate.month - 1]).mousedown(function () {
                        return !1
                    }).click(function (t) {
                        t.stopPropagation(), n.css({display: "none"}), s.css({display: "inline-block"})
                    }),
                    r = t("<span/>").html(e.options.persianNumbers ? e.jDateFunctions.toPersianNums(e.persianDate.year) : e.persianDate.year).mousedown(function () {
                        return !1
                    }).click(function (t) {
                        t.stopPropagation(), s.css({display: "none"}), n.css({display: "inline-block"}), n.scrollTop(70)
                    });
                ...
                var d = function (s, o) {
                    ...
                    t.each(e.options.selectableYears || (r ? d.reverse() : d), function (a, s) {
                        ...
                        s == e.persianDate.year && i.addClass("selected"), i.attr("value", s), i.bind("click", function () {
                            e.persianDate.date = 1, e.persianDate.year = parseInt(s), _endDate.year != s && 9999 != _endDate.year || (e.persianDate.month = _endDate.month), _startDate.year != s && 9999 != _startDate.year || (e.persianDate.month = _startDate.month), e.render()
                        }), r ? n.prepend(i) : n.append(i)
                    })
                };
                for (d(), i = 1; i <= 12; i++) {
                    ...
                    i == e.persianDate.month && h.addClass("selected"), h.data("month", {
                        month: l,
                        monthNum: i
                    }), h.hasClass("disableMonth") || h.bind("click", function () {
                        e.persianDate.date = 1, e.persianDate.month = t(this).data("month").monthNum, e.render();
                        e.el.val(e.persianDate.year.toString() + "/" + e.persianDate.month);
                        e.hide()
                    }), s.append(h)
                }
                n.bind("scroll", function () {
                    null == e.options.selectableYears && (c = t(this).find("li").length, firstYear = parseInt(t(this).children("li:first").val()), lastYear = parseInt(t(this).children("li:last").val()), lisHeight = c / 3 * (t(this).find("li:first").height() + 4), _com = 500 * t(this).scrollTop().toString().length, t(this).scrollTop() < 100 * _com.toString().length && firstYear >= 1 && d(firstYear, 0), _com = 100 * t(this).scrollTop().toString().length, lisHeight - t(this).scrollTop() > -_com && lisHeight - t(this).scrollTop() < _com && (d(0, lastYear), t(this).scrollTop(t(this).scrollTop() - 50)), t(this).scrollTop() < _com.toString().length && firstYear >= 30 && t(this).scrollTop(100 * _com.toString().length))
                }), _monthYear.append(s).append(n);
                var p = t('<div class="monthYear" />').append(o).append("<span>&nbsp;&nbsp;</span>").append(r);
                _head.append(p), _prev = t('<div class="prevArrow" />').html(this.options.prevArrow).attr("title", "ماه قبل"), null == e.options.startDate || e.persianDate.parse(e.options.startDate).year < e.persianDate.year || e.persianDate.parse(e.options.startDate).month < e.persianDate.month ? (_prev.bind("click", function () {
                    e.persianDate.addMonth(-1), e.render()
...
var persianDate = function () {
    function t() {
        this.months = ["فروردین", "اردیبهشت", "خرداد", "تیر", "مرداد", "شهریور", "مهر", "آبان", "آذر", "دی", "بهمن", "اسفند"], this.dowTitle = ["شنبه", "یکشنبه", "دوشنبه", "سه شنبه", "چهارشنبه", "پنج شنبه", "جمعه"], this.year = 1300, this.month = 1, this.date = 1, this.day = 1, this.gDate = new Date
    }

    return t.prototype = {
        now: function () {
            return (new jDateFunctions).gregorian_to_jalali(new Date)
        }, addDay: function (e) {
            ...
            for (var e = t > 0 ? t : -t, a = 0; a < e; a++) t > 0 ? this.month += 1 : this.month -= 1, 13 == this.month ? (this.month = 1, this.addYear(1)) : 0 == this.month && (this.month = 12, this.addYear(-1));
            return this
        }, addYear: function (t) {
            ...
            arr = e.split("/"), y = arr[0], m = arr[1], d = arr[2];
            var a = new t;
            return jdf = new jDateFunctions, a.year = parseInt(y), a.month = parseInt(m), a.date = parseInt(d), a.day = jdf.getWeekday(a), a.gDate = jdf.jalali_to_gregorian(a), a
        }, toString: function (t) {
    ...
    return t.prototype = {
        toPersianNums: function (t) {
            ...
            return res
        }, gregorian_to_jalali: function (t) {
            return gy = t.getFullYear(), gm = t.getMonth() + 1, gd = t.getDate(), g_d_m = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334], gy > 1600 ? (jy = 979, gy -= 1600) : (jy = 0, gy -= 621), gy2 = gm > 2 ? gy + 1 : gy, days = 365 * gy + parseInt((gy2 + 3) / 4) - parseInt((gy2 + 99) / 100) + parseInt((gy2 + 399) / 400) - 80 + gd + g_d_m[gm - 1], jy += 33 * parseInt(days / 12053), days %= 12053, jy += 4 * parseInt(days / 1461), days %= 1461, days > 365 && (jy += parseInt((days - 1) / 365), days = (days - 1) % 365), jm = days < 186 ? 1 + parseInt(days / 31) : 7 + parseInt((days - 186) / 30), jd = 1 + (days < 186 ? days % 31 : (days - 186) % 30), t = new Date, pd = new persianDate, pd.year = jy, pd.month = jm, pd.date = jd, pd.gDate = t, pd
        }, jalali_to_gregorian: function (t) {
            ...
            return this.jalali_to_gregorian(t)
        }, getWeekday: function (t) {
            return [1, 2, 3, 4, 5, 6, 0][this.jalali_to_gregorian(t).getDay()]
        }, getLastDayOfMonth: function (t) {
            return y = t.year, m = t.month, m >= 1 && m <= 6 ? 31 : m >= 7 && m < 12 ? 30 : this.isLeapYear(y) ? 30 : 29
        }, isLeapYear: function (t) {
            return b = t % 33, (t > 1342 ? [1, 5, 9, 13, 17, 22, 26, 30] : [1, 5, 9, 13, 17, 21, 26, 30])._indexOf(b) > -1
        }
    }, t
}();